...
import {ISemaphoreGroups} from "./ISemaphorePayboxGroups.sol";
...
abstract contract SemaphorePayboxGroups is ISemaphoreGroups {
    ...
    function _withdrawFromBox(
        uint256 groupId,
        uint amount
    )internal virtual onlyGroupAdmin(groupId) returns (uint256) {
        address payable boxOwner = payable(admins[groupId]);
        boxOwner.transfer(amount);
        return amount;
    }
